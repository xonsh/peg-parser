...
def test_invalid_statements(python_parse_file, python_parse_str, tmp_path, source, message, start, end):
    parse_invalid_syntax(
        python_parse_file, python_parse_str, tmp_path, source, SyntaxError, message, start, end
    )
...
def test_invalid_call_arguments(python_parse_file, python_parse_str, tmp_path, source, message, start, end):
    parse_invalid_syntax(
        python_parse_file,
        python_parse_str,
        tmp_path,
...
@pytest.mark.parametrize(
    "source, message, start, end",
    [
        pytest.param("'a' = 1", "cannot assign to literal", (1, 1), (1, 4), marks=pytest.mark.xfail),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            (4, 1),
            (4, 5),
            marks=pytest.mark.skipif(sys.version_info < (3, 11), reason="Syntax only permitted on 3.11+"),
        ),
        (
        ...
        pytest.param(
            ...
            (3, 9),
            (3, 36),
            marks=pytest.mark.skipif(sys.version_info < (3, 11), reason="Syntax unsupported before 3.11+"),
        ),
        (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            (2, 1),
            (2, 5),
            marks=pytest.mark.skipif(sys.version_info < (3, 12), reason="Syntax only permitted on 3.12+"),
        ),
        (
            "def f() -> None:\npass",
            IndentationError,
