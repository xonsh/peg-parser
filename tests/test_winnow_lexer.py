...
import pytest
from winnow_parser import Token as t  # noqa: N813
from winnow_parser import TokInfo, tokenize


def ensure_tuple(seq, inp: str) -> str:
    if isinstance(seq, TokInfo):
        seq = (seq.typ, seq.get_string(inp), seq.start[1])
...
def assert_tokens_equal(expected, obtained, inp: str):
    """Asserts that two token sequences are equal."""
    left = [ensure_tuple(item, inp) for item in expected]
    right = [ensure_tuple(item, inp) for item in obtained]
    if diff := "\n".join(difflib.unified_diff(left, right, "expected", "obtained")):
        print("\n".join(difflib.ndiff(left, right)))
    return not diff


def lex_input(inp: str) -> list[TokInfo]:
    # skip the NEWLINE, ENDMARKER tokens for easier testing

    tokens = tokenize(inp)
    if tokens[-1].typ == t.ENDMARKER:
        tokens.pop()
    if tokens[-1].typ == t.NEWLINE:
        tokens.pop()
    return tokens
...
def check_tokens(inp: str, *exp):
    obs = lex_input(inp)
    return assert_tokens_equal(exp, obs, inp)


def check_tokens_subproc(inp, exp, stop=-1):
    full_inp = f"$[{inp}]"
    obs = lex_input(full_inp)[1:stop]
    return assert_tokens_equal(exp, obs, full_inp)
