...
class GeneratedParser(Parser):
    ...
    def meta(self) -> Optional[MetaTuple]:
        ...
        self._reset(mark)
        if (self.expect("@")) and (a := self.name()) and (b := self.name()) and (self.expect("NEWLINE")):
    ...
    def named_item(self) -> Optional[NamedItem]:
        ...
        if cut:
            return None
        cut = False
        if (name := self.name()) and (self.expect("=")) and (cut := True) and (item := self.item()):
    ...
    def item(self) -> Optional[Item]:
        ...
        if (atom := self.atom()) and (self.expect("+")):
            return Repeat1(atom)
        self._reset(mark)
        if (sep := self.atom()) and (self.expect(".")) and (node := self.atom()) and (self.expect("+")):
    ...
    def action(self) -> Optional[str]:
        ...
        if (
            ...
            return join_tokens(target_atoms)
        self._reset(mark)
        if cut:
            return None
        return None
    ...
    @memoize
    def target_atoms(self) -> Optional[list]:
    ...
    @memoize
    def target_atom(self) -> Optional[list]:
        # target_atom: "{" ~ target_atoms? "}" | "[" ~ target_atoms? "]" | FSTRING_START target_fstring_middle* FSTRING_END | NAME "*" | (NAME | NUMBER | STRING | "?" | ":") | !"}" !"]" OP
        mark = self._mark()
        cut = False
        if (
            (l := self.expect("{"))
        ...
        if (l := self.fstring_start()) and (m := self._loop0_1(),) and (r := self.fstring_end()):
            return [
                TokenInfo(
                    string=l.string + "".join(m) + r.string, start=l.start, end=r.end, line=l.line, type=1
                )
            ]
        self._reset(mark)
        if (a := self.name()) and (b := self.expect("*")):
            return [a, b]
        self._reset(mark)
        if a := self._tmp_2():
            return [a]
    ...
    def target_fstring_middle(self) -> Optional[str]:
        ...
        self._reset(mark)
        if target_atom := self.target_atom():
            return join_tokens(target_atom)
        self._reset(mark)
        return None
