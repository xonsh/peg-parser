...
RE_STRINGPREFIX = LazyObject(lambda: re.compile(StringPrefix), globals(), "RE_STRINGPREFIX")
...
def call_split_lines(x, lineno=None, col=None):
    ...
    """
    return ast.Call(
        func=ast.Attribute(value=x, attr="splitlines", ctx=ast.Load(), lineno=lineno, col_offset=col),
        args=[],
        keywords=[],
        starargs=None,
        kwargs=None,
...
def ensure_list_from_str_or_list(x, lineno=None, col=None):
    ...
    return ast.IfExp(
        test=ast.Call(
            func=ast.Name(id="isinstance", ctx=ast.Load(), lineno=lineno, col_offset=col),
...
class BaseParser:
    ...
    def xonsh_pathsearch(self, pattern, pymode=False, lineno=None, col=None):
        ...
        pymode = ast.const(pymode, lineno=lineno, col_offset=col)
        ...
        if not searchfunc.startswith("@") and "f" in searchfunc:
            ...
            if pattern is None:
                try:
                    pattern = FStringAdaptor(pattern_as_str, "f", filename=self.lexer.fname).run()
                except SyntaxError as e:
                    self._set_error(str(e), self.currloc(lineno=lineno, column=col))
        else:
            pattern = ast.const(pattern, lineno=lineno, col_offset=col)
        pathobj = False
        ...
        pathobj = ast.const(value=pathobj, lineno=lineno, col_offset=col)
        return xonsh_call(
    ...
    def p_attr_name_alone(self, p):
        ...
        p1 = p[1]
        p[0] = ast.Name(id=p1.value, ctx=ast.Load(), lineno=p1.lineno, col_offset=p1.lexpos)

    def p_attr_name_with(self, p):
        """attr_name_with : name attr_period_name_list"""
        p1 = p[1]
        name = ast.Name(id=p1.value, ctx=ast.Load(), lineno=p1.lineno, col_offset=p1.lexpos)
        p2 = p[2]
        p0 = ast.Attribute(
            value=name,
            attr=p2[0],
    ...
    def p_typedargslist_kwarg(self, p):
        ...
        p[0] = ast.arguments(args=[], vararg=None, kwonlyargs=[], kw_defaults=[], kwarg=p[2], defaults=[])
    ...
    def p_typedargslist_times4_comma(self, p):
        ...
        # *, **kwargs
        p0 = ast.arguments(args=[], vararg=None, kwonlyargs=[], kw_defaults=[], kwarg=p[2], defaults=[])
        p[0] = p0
    ...
    def _set_args_def(self, argmts, vals, kwargs=False):
        args, defs = (argmts.kwonlyargs, argmts.kw_defaults) if kwargs else (argmts.args, argmts.defaults)
    ...
    def p_expr_stmt_testlist_assign(self, p):
        ...
        if p2 is None and len(p1) == 1:
            ...
            p[0] = ast.Assign(targets=p1 + p2[:-1], value=p2[-1], lineno=lineno, col_offset=col)
    ...
    def store_star_expr(self, p1, p2, targs, rhs):
        ...
        p0 = ast.Assign(targets=p1, value=rhs, lineno=p1[0].lineno, col_offset=p1[0].col_offset)
        return p0
    ...
    def p_import_from(self, p):
        ...
        lvl = len(p1) - len(mod)
        mod = mod or None
        p[0] = ast.ImportFrom(module=mod, names=p[3], level=lvl, lineno=lineno, col_offset=col)
    ...
    def p_with_stmt_w5(self, p):
        ...
        p1 = p[1]
        p[0] = [ast.With(items=[p[2]], body=p[4], lineno=p1.lineno, col_offset=p1.lexpos)]

    def p_with_stmt_p6(self, p):
        """with_stmt : with_tok with_item comma_with_item_list COLON suite"""
        p1 = p[1]
        p[0] = [ast.With(items=[p[2]] + p[3], body=p[5], lineno=p1.lineno, col_offset=p1.lexpos)]
    ...
    def p_with_bang_stmt_many_suite(self, p):
        ...
        for item in items:
            ...
            item.context_expr = xonsh_call("__xonsh__.enter_macro", margs, lineno=l, col=c)
        body = [ast.Pass(lineno=p5.lineno, col_offset=p5.col_offset)]
        p[0] = [ast.With(items=items, body=body, lineno=p1.lineno, col_offset=p1.lexpos)]
    ...
    def p_except_clause_e2(self, p):
        ...
        p1 = p[1]
        p[0] = ast.ExceptHandler(type=None, name=None, lineno=p1.lineno, col_offset=p1.lexpos)

    def p_except_clause(self, p):
        """except_clause : except_tok test as_name_opt"""
        p1 = p[1]
        p[0] = ast.ExceptHandler(type=p[2], name=p[3], lineno=p1.lineno, col_offset=p1.lexpos)
    ...
    def p_rawsuite_indent(self, p):
        ...
        s = textwrap.dedent(s)
        p[0] = ast.const(s, lineno=beg[0], col_offset=beg[1])
    ...
    def p_test_o5(self, p):
        ...
        p[0] = ast.IfExp(test=p[3], body=p[1], orelse=p[5], lineno=self.lineno, col_offset=self.col)
    ...
    def p_or_test(self, p):
        ...
        if p2 is None:
            ...
            lineno, col = lopen_loc(p1)
            p0 = ast.BoolOp(op=p2[0], values=[p[1]] + p2[1::2], lineno=lineno, col_offset=col)
        p[0] = p0
    ...
    def p_and_test(self, p):
        ...
        if p2 is None:
            ...
            lineno, col = lopen_loc(p1)
            p0 = ast.BoolOp(op=p2[0], values=[p1] + p2[1::2], lineno=lineno, col_offset=col)
        p[0] = p0
    ...
    def p_not_test_not(self, p):
        ...
        p[0] = ast.UnaryOp(op=ast.Not(), operand=p[2], lineno=self.lineno, col_offset=self.col)
    ...
    def p_star_expr(self, p):
        ...
        p1 = p[1]
        p[0] = ast.Starred(value=p[2], ctx=ast.Load(), lineno=p1.lineno, col_offset=p1.lexpos)
    ...
    def p_shift_arith_expr(self, p):
        ...
        p[0] = [ast.BinOp(left=None, op=op, right=p[2], lineno=p1.lineno, col_offset=p1.lexpos)]
    ...
    def p_arith_expr_many(self, p):
        ...
        if len(p2) == 2:
            lineno, col = lopen_loc(p1)
            p0 = ast.BinOp(left=p1, op=p2[0], right=p2[1], lineno=lineno, col_offset=col)
        else:
            left = p1
            for op, right in zip(p2[::2], p2[1::2], strict=False):
                locer = left if left is p1 else op
                lineno, col = lopen_loc(locer)
                left = ast.BinOp(left=left, op=op, right=right, lineno=lineno, col_offset=col)
            p0 = left
        p[0] = p0
    ...
    def p_factor_unary(self, p):
        ...
        p[0] = ast.UnaryOp(op=op, operand=p[2], lineno=self.lineno, col_offset=p1.lexpos)
    ...
    def _list_or_elts_if_not_real_tuple(self, x):
        if isinstance(x, ast.Tuple) and not (hasattr(x, "_real_tuple") and x._real_tuple):
            rtn = x.elts
        else:
            rtn = [x]
        return rtn
        ...
        for trailer in trailers:
            if isinstance(
                ...
                if trailer == "?":
                    ...
                    p0 = xonsh_superhelp(leader, lineno=leader.lineno, col=leader.col_offset)
                else:
                    p0 = ast.Attribute(
                        value=leader,
                        attr=trailer,
    ...
    def p_atom_lparen(self, p):
        ...
        if p2 is None:
            ...
            p0 = ast.Tuple(elts=[], ctx=ast.Load(), lineno=self.lineno, col_offset=self.col)
            p0._real_tuple = True
    ...
    def p_atom_lbraket(self, p):
        ...
        p1, p1_tok = p1.value, p1
        if p2 is None:
            p0 = ast.List(elts=[], ctx=ast.Load(), lineno=self.lineno, col_offset=self.col)
    ...
    def p_atom_none(self, p):
        """atom : none_tok"""
        p1 = p[1]
        p[0] = ast.const(value=None, lineno=p1.lineno, col_offset=p1.lexpos)

    def p_atom_true(self, p):
        """atom : true_tok"""
        p1 = p[1]
        p[0] = ast.const(value=True, lineno=p1.lineno, col_offset=p1.lexpos)

    def p_atom_false(self, p):
        """atom : false_tok"""
        p1 = p[1]
        p[0] = ast.const(value=False, lineno=p1.lineno, col_offset=p1.lexpos)

    def p_atom_pathsearch(self, p):
        """atom : SEARCHPATH"""
        p[0] = self.xonsh_pathsearch(p[1], pymode=True, lineno=self.lineno, col=self.col)
    ...
    def p_string_literal(self, p):
        ...
        if "p" in prefix and "f" in prefix:
            ...
            if s is None:
                try:
                    s = FStringAdaptor(value_without_p, new_pref, filename=self.lexer.fname).run()
                except SyntaxError as e:
                    self._set_error(str(e), self.currloc(lineno=p1.lineno, column=p1.lexpos))
            ...
            if s is None:
                try:
                    s = FStringAdaptor(p1.value, prefix, filename=self.lexer.fname).run()
                except SyntaxError as e:
                    self._set_error(str(e), self.currloc(lineno=p1.lineno, column=p1.lexpos))
            s = ast.increment_lineno(s, p1.lineno - 1)
            if "r" in prefix:
    ...
    def p_number(self, p):
        ...
        p1 = p[1]
        p[0] = ast.const(
            ast.literal_eval(p1.value.replace("_", "")),
            lineno=p1.lineno,
            col_offset=p1.lexpos,
        )
    ...
    def p_trailer_bang_lparen(self, p):
        ...
        for beg, end in zip(begins, ends, strict=False):
            ...
            node = ast.const(s, lineno=beg[0], col_offset=beg[1])
            elts.append(node)
    ...
    def p_testlist_test(self, p):
        """testlist : test"""
        p1 = p[1]
        if isinstance(p1, ast.Tuple) and (hasattr(p1, "_real_tuple") and p1._real_tuple and p1.elts):
    ...
    def p_testlist_many(self, p):
        ...
        if isinstance(p1, ast.List) or (
            ...
            lineno, col = lopen_loc(p1)
            p1 = ast.Tuple(elts=[p1], ctx=ast.Load(), lineno=p1.lineno, col_offset=p1.col_offset)
        else:
    ...
    def p_dictorsetmaker_comp(self, p):
        ...
        if isinstance(p1, list) and len(p1) == 2:
            ...
            p[0] = ast.SetComp(elt=p1, generators=comps, lineno=self.lineno, col_offset=self.col)
    ...
    def _get_envvars(self, p, lineno, col):
        ...
        for subproc in p:
            if hasattr(subproc, "_xenvvars"):
                ...
                envs.elts.append(
                    ast.Constant(value=None, lineno=subproc.lineno, col_offset=subproc.col_offset)
                )
    ...
    def _dollar_rules(self, p):
        ...
        if lenp == 3:  # $NAME
            ...
            p0 = ast.Subscript(value=xenv, slice=idx, ctx=ast.Load(), lineno=lineno, col_offset=col)
        elif p1 == "$(":
            p0 = xonsh_call("__xonsh__.subproc_captured_stdout", p2, lineno=lineno, col=col)
        elif p1 == "!(":
            p0 = xonsh_call("__xonsh__.subproc_captured_object", p2, lineno=lineno, col=col)
        elif p1 == "![":
            p0 = xonsh_call("__xonsh__.subproc_captured_hiddenobject", p2, lineno=lineno, col=col)
        elif p1 == "$[":
        ...
        if envs is not None:
            p0.keywords.append(ast.keyword(arg="envs", value=envs, lineno=lineno, col_offset=col))
    ...
    def _envvar_getter_by_name(self, var, lineno=None, col=None):
        ...
        return ast.Call(
            func=func,
            args=[
                ast.const(var, lineno=lineno, col_offset=col),
                ast.const("", lineno=lineno, col_offset=col),
            ],
            keywords=[],
            lineno=lineno,
            col_offset=col,
    ...
    def _envvar_by_name(self, var, lineno=None, col=None):
        ...
        xenv = load_attribute_chain("__xonsh__.env", lineno=lineno, col=col)
        idx = ast.Index(value=ast.const(var, lineno=lineno, col_offset=col))
        ...
        for arg in args:
            ...
            if action == "append":
                if currlist is None:
                    ...
                    cliargs = binop(cliargs, ast.Add(), currlist, lineno=lineno, col=col)
    ...
    def p_pipe(self, p):
        ...
        pipe : PIPE
             ...
             | WS PIPE WS
        """
        p[0] = ast.const("|", lineno=self.lineno, col_offset=self.col)
    ...
    def p_subproc_atoms_subshell(self, p):
        ...
        subcmd = subcmd.strip() + "\n"
        p0 = [
            ast.const("xonsh", lineno=l, col_offset=c),
            ast.const("-c", lineno=l, col_offset=c),
            ast.const(subcmd, lineno=l, col_offset=c),
        ]
        for arg in p0:
    ...
    def _append_subproc_bang_empty(self, p):
        ...
        p3 = p[3]
        node = ast.const("", lineno=p3.lineno, col_offset=p3.lexpos + 1)
    ...
    def p_subproc_atom_uncaptured(self, p):
        ...
        p1 = p[1]
        p0 = xonsh_call("__xonsh__.subproc_uncaptured", args=p[2], lineno=p1.lineno, col=p1.lexpos)
    ...
    def p_subproc_atom_pyenv_lookup(self, p):
        ...
        p0 = ast.Call(
            func=func,
            args=[p[2], ast.const("", lineno=lineno, col_offset=col)],
            keywords=[],
            lineno=lineno,
            col_offset=col,
        )
    ...
    def p_subproc_atom_subproc_inject(self, p):
        ...
        """
        p1, p2 = p[1], p[2]
        p0 = xonsh_call("__xonsh__.subproc_captured_inject", p2, lineno=p1.lineno, col=p1.lexpos)
        p0._cliarg_action = "extend"
    ...
    def p_subproc_atom_redirect(self, p):
        ...
        subproc_atom : GT WS subproc_atom
                     ...
                     | IOREDIRECT2
        """
        operator = ast.const(p[1], lineno=self.lineno, col_offset=self.col)
    ...
    def p_subproc_atom_arg(self, p):
        ...
        if isinstance(p1, list):
            ...
            p0 = xonsh_call("__xonsh__.glob", args=[p1], lineno=p1.lineno, col=p1.col_offset)
            p0._cliarg_action = "extend"
        else:
    ...
    def p_subproc_arg_part_brackets(self, p):
        ...
        p2 = p[2]
        p3 = p[3]
        p1 = ast.const(p1.value, lineno=p1.lineno, col_offset=p1.lexpos)
        p3 = ast.const(p3.value, lineno=p3.lineno, col_offset=p3.lexpos)
    ...
    def p_subproc_arg_part_brackets_empty(self, p):
        ...
        p2 = p[2]
        p1 = ast.const(p1.value, lineno=p1.lineno, col_offset=p1.lexpos)
        p2 = ast.const(p2.value, lineno=p2.lineno, col_offset=p2.lexpos)
    ...
    def p_subproc_arg_part(self, p):
        ...
        p1 = p[1]
        p[0] = ast.const(p1.value, lineno=p1.lineno, col_offset=p1.lexpos)
    ...
    def p_envvar_assign(self, p):
        ...
        p1, p2 = p[1], p[2]
        p[0] = ast.Dict(
            keys=[ast.Constant(value=p1.value[1:], lineno=p1.lineno, col_offset=p1.lexpos)],
            values=[p2],
            lineno=p1.lineno,
            col_offset=p1.lexpos,
        )
    ...
    def p_error(self, p):
        if p is None:
            ...
            if isinstance(p.value, BaseException):
                ...
                self._parse_error(p.value, self.currloc(lineno=p.lineno, column=p.lexpos))
        else:
            msg = (f"code: {p.value}",)
